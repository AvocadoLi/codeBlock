<template>
  <div class="background-container">
    ...
    <div class="relative z-3 h-screen w-screen">
      <slot />
    </div>
  </div>
</template>
    ...
    if (dot.restCounter > 0) {
      ...
      if (dot.restCounter === 0) {
        if (Math.random() < FLICKER_CHANCE * 5) {
          // 设置新的目标亮度
          dot.targetBrightness = Math.random() < 0.5 ? MAX_BRIGHTNESS : MIN_BRIGHTNESS
      ...
      if (Math.abs(dot.brightness - dot.targetBrightness) < TRANSITION_SPEED) {
        ...
        if (Math.random() < CONTINUE_FLICKER_CHANCE) {
          ...
          dot.targetBrightness =
            dot.targetBrightness === MAX_BRIGHTNESS ? MIN_BRIGHTNESS : MAX_BRIGHTNESS
        } else {
          // 停止闪烁并进入休息状态
          dot.flickerSpeed = 0
